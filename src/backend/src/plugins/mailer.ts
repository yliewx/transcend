...
// Create a plugin that adds nodemailer to the fastify instance
export default fp(async function setupMailer(server: FastifyInstance) {
  ...
  // Add sendEmailOtp to mailer service
  ...
  // // Add the transporter to the fastify instance
  // server.decorate('mailer', {
  //   transporter,
  //   sendEmailOtp: async (email: string, otpToken: string): Promise<boolean> => {
  //     try {
  //       const info = await transporter.sendMail({
  //         from: `"${process.env.EMAIL_FROM_NAME || 'App Security'}" <${process.env.EMAIL_FROM_ADDRESS || 'security@yourapp.com'}>`,
  //         to: email,
