...
export default fp(async function setupJwt(server: FastifyInstance) {
  ...
  //   secret: (request: FastifyRequest, token: string) => {
  ...
  server.decorate("authenticate", async (request, reply) => {
    try {
      ...
      // Decode JWT token and attach the decoded user data to the request
      request.user = await server.jwt.verify<AuthTokenPayload>(accessToken);
      server.log.info('inside authenticate middleware');
    }
    catch (err) {
      reply.status(401).send({ error: 'Unauthorized' });
    }
